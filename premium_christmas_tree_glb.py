...
def export_premium_glb():
    """Export as premium GLB"""
    output_dir = Path("/Volumes/Folk_DAS/Apps/MacroAI/3D_Assets/christmas")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    glb_path = output_dir / "christmas_tree_premium.glb"
    ...
    print(f"✅ Premium GLB exported: {glb_path}")
    return str(glb_path)

def convert_glb_to_usdz(glb_path):
    """Convert GLB to USDZ using Apple's tools"""
    glb_path = Path(glb_path)
    usdz_path = glb_path.with_suffix('.usdz')
    
    # Use Apple's usdconvert tool
    cmd = f"/Applications/Xcode.app/Contents/Developer/usr/bin/usdconvert {glb_path} {usdz_path}"
...
def main():
    print("🎄 Creating premium Christmas tree...")
    ...
    glb_path = export_premium_glb()
    print("✅ Premium GLB exported")
    
    usdz_path = convert_glb_to_usdz(glb_path)
    if usdz_path:
        print(f"🎉 Premium USDZ ready: {usdz_path}")
    else:
        print("⚠️ USDZ conversion failed, using GLB")
